...
export default function AssociateAssignPreRegistered() {
  ...
  const [prefSpeed] = useState<string>("24 horas");
  const [prefTime, setPrefTime] = useState<string>("Mañana (8:00 AM – 10:00 AM)");
    ...
    if (prefilledPhone) {
      const fetchPrefilledUser = async () => {
        ...
        try {
          ...
          setFoundUser({ id: uDoc.id, ...uData } as any);
          if (uData.preferredTime) {
            setPrefTime(uData.preferredTime);
          }
        } catch (err: any) {
  ...
  return (
    <div className="flex-1 flex flex-col w-full h-full">
      ...
      <div className="flex-1 flex flex-col px-6 w-full max-w-md mx-auto">
        ...
        {foundUser && !scanning && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex-1 flex flex-col items-center justify-start text-center pb-12 w-full pt-2">
             <div className="w-full space-y-5">
               <div>
                  <div className="inline-flex items-center justify-center px-3.5 py-1 bg-green-50 text-green-700 rounded-full text-xs font-bold uppercase tracking-wider mb-3 border border-green-100 space-x-1.5">
                     <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
                     <span>Cliente Encontrado</span>
                  </div>
                  <h1 className="text-2xl font-bold text-slate-900 leading-tight truncate px-2 block">{foundUser.name}</h1>
                  <p className="text-sm text-gray-400 mt-1 font-mono">{foundUser.phone}</p>
               </div>

               {/* Selectable Options (Horario Preferido) */}
               <div className="w-full bg-slate-50/70 border border-slate-100 rounded-2xl p-4 space-y-3 text-left">
                 {/* Horario de Entrega Preferido */}
                 <div className="space-y-2">
                   <label className="text-[10px] uppercase tracking-widest font-extrabold text-slate-400 block">Horario de Entrega Preferido</label>
                   <div className="grid grid-cols-2 gap-2.5">
                     ...
                     <motion.button
                       ...
                       className={`relative py-2.5 px-3 rounded-xl border text-center transition-all duration-200 focus:outline-none flex flex-col items-center justify-center cursor-pointer ${
                         prefTime === "Mañana (8:00 AM – 10:00 AM)"
                           ? "bg-blue-50/80 border-[#0f55d8] border-2 shadow-2xs"
                           : "bg-white border-slate-200 hover:border-slate-300"
                       }`}
                     >
                       <span className={`font-black text-sm tracking-tight block ${prefTime === "Mañana (8:00 AM – 10:00 AM)" ? "text-[#0f55d8]" : "text-slate-800"}`}>
                         Mañana
                       </span>
                       <span className="text-[11px] leading-tight text-gray-500 font-bold block mt-0.5">
                         8 AM-10 AM
                       </span>
                     </motion.button>
                     ...
                     >
                       <span className={`font-black text-sm tracking-tight block ${prefTime === "Noche (8:00 PM – 10:00 PM)" ? "text-[#0f55d8]" : "text-slate-800"}`}>
                         Noche
                       </span>
                       <span className="text-[11px] leading-tight text-gray-500 font-bold block mt-0.5">
                         8 PM-10 PM
               ...
               <div className="pt-2 space-y-2">
                  <button
                    ...
                    className="w-full bg-[#0f55d8] text-white py-4 px-5 rounded-2xl font-bold flex flex-col items-center justify-center hover:bg-[#0c4ab9] transition-all disabled:opacity-50 text-base cursor-pointer select-none outline-none shadow-sm"
                  >
                    {prefilledBagId ? (
                  ...
                  {prefilledBagId && (
                    <button
                      ...
                      className="w-full py-3 rounded-xl border border-dashed border-gray-300 text-gray-500 font-bold text-xs hover:bg-gray-50 hover:text-gray-700 transition-all outline-none flex items-center justify-center gap-2 cursor-pointer"
                    >
